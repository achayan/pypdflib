...
class Image(Widget):
    ...
    def __init__(self, image_file=None, width=None, height=None, scale_x=None, scale_y=None,padding_bottom=10):
        super(Image,self).__init__()
        self.image_file =  ImageConvert.open(image_file)
        self.imgae_info = self.image_file.size
        self.width =  self.imgae_info[0]
        self.height =  self.imgae_info[1]
        self.padding_bottom = padding_bottom
        background = ImageConvert.new('RGBA', self.image_file.size, (255, 255, 255))
        background.paste(self.image_file)
        newImage = background.convert('RGB').convert('P', palette=ImageConvert.ADAPTIVE)
        getNewFileName='%s_.png'%(os.path.splitext(image_file)[0])
        newImage.save(getNewFileName)
        self.image_file = getNewFileName

    def clearTempImage(self,image_file=None):
        clearCmd = 'rm %s'%image_file
        os.system(clearCmd)
